...
<!-- row opened -->
						<div class="row">
											...
											<table id="example" class="table table-bordered key-buttons text-wrap">
												...
												$data = $this->db->from("rtransaction_main")->where("rid",$_SESSION['rid'])->where('date >=',$from)->where('date <=',$to)->order_by('id',"DESC")->get()->result();
												
                                                                                                ?>
												<tbody>
												    <?php $ind = 1; ?>
												    <?php foreach($data as $dat) :
                                                        $txntype = $this->db->get_where('main_wallet', array('site' => $dat->rid,'txnid'=>$dat->txnid))->row()->txntype;
                                                    ?>
													<tr>
														<td><?php echo $ind; ?></td>
