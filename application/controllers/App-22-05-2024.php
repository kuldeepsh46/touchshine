...
class App extends CI_Controller {
    ...
    public function rechargev2op_lapu() {
        $data = $this->db->get('rechargev2op')->result();
        print_r(json_encode(array('status' => "SUCCESS", 'data' => $data, 'error_code' => 0)));
        exit();
    }
    ...
    public function dorecharge() {
        ...
        $amount = $this->input->post('amount');
        $operator = $this->input->post('operator');
        $circle = $this->input->post('circlecode');
            ...
            if ($amount <= 0) {
                ...
                if ($amount >= $udata->wallet) {
                    ...
                    if ($amount >= $rdata->wallet) {
                        ...
                        // Get selected Recharge API --- Edit By HarshaWeb ---
                        $recharge_api = $this->db->get_where('settings', array('name' => "recharge_api"))->row()->value;

                        if ($recharge_api == "CYRUS") {
                            $responce = recharge($api_key, $mobile, $amount, $opcode, $circle, $txnid);
                            $main = $responce;
                        } else {
                            $responce = rechargeMobotics("a9bb140c-5298-43b7-995e-af293d162fee", $mobile, $amount, $opcode, $txnid);
                            $main = $responce;
                        }
