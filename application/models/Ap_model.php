...
class ap_model extends CI_Model
{
  ...
  {
    $ispercent = $this->db->get_where('icapcharge', array('package' => $utype, 'froma<=' => $amount, 'toa>=' => $amount))->row()->percent;
    if ($ispercent == 0) {
      return false;
    } else {
      return true;
  ...
  public function adhaarPayCharge($utype, $amount)
  {
    $adhaarPayCharge = $this->db->get_where('icapcharge', array('package' => $utype, 'froma<=' => $amount, 'toa>=' => $amount))->row()->amount;
    return $adhaarPayCharge;
    // return 2;
  }
  ...
  public function charge($uid, $amount, $txnid, $site)
  {
    $main_user = $uid;
    $commision = array();
    ...
    if ($this->isPercent($type, $amount)) {
      $main_commision = array_sum($commision) + ($this->adhaarPayCharge($type, $amount) * ($amount / 100));
    } else {
      $main_commision = array_sum($commision) + $this->adhaarPayCharge($type, $amount);
    }
    ...
    while ($uid != 0) {
      if ($this->isOwner($uid)) {
        ...
        $parent_type =  $this->uType($parent_id);
        if ($this->isPercent($type, $amount)) {
          $main_commision = $this->adhaarPayCharge($type, $amount) * ($amount / 100);
        } else {
          $main_commision = $this->adhaarPayCharge($type, $amount);
        }
        ...
        if ($this->isPercent($parent_type, $amount)) {
          $parent_commision = ($this->adhaarPayCharge($parent_type, $amount) * ($amount / 100));
        } else {
          $parent_commision = $this->adhaarPayCharge($parent_type, $amount);
        }
      ...
      foreach ($user_n_com as $key => $value) {
        foreach ($value as $key => $value) {
          ...
          $data = ['uid' => $key, 'amount' => $value, 'txnid' => $txnid, 'opening' => $openingBal, 'closing' => $closingBal, 'site' => $site, 'type' => 'I-AP-CHARGE', 'txntype' => $txntype];
          $checkTxn = $this->db->get_where('wallet', array('txnid' => $txnid, 'uid' => $key, 'txntype' => $txntype, 'type' => 'I-AP-CHARGE'))->row()->txnid; //check if there is any entry available with same transaction id(txnid), so that we can save from doind clone entry
          if ($checkTxn) {
          } else {
