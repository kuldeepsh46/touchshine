<?php
class qtransfer_model extends CI_Model
{
    ...
    {
        $ispercent = $this->db->get_where('qtransfer', array('package' => $utype, 'froma<=' => $amount, 'toa>=' => $amount))->row()->percent;
        if ($ispercent == 0) {
            return false;
        } else {
            return true;
    ...
    public function qTransfer($utype, $amount)
    {
        $qTransfer = $this->db->get_where('qtransfer', array('package' => $utype, 'froma<=' => $amount, 'toa>=' => $amount))->row()->amount;
        return $qTransfer;
        // return 2;
    }
    ...
    public function charge($uid, $amount, $txnid, $site)
    {
        ...
        if ($this->isPercent($type, $amount)) {
            $main_commision = array_sum($commision) + ($this->qTransfer($type, $amount) * ($amount / 100));
        } else {
            $main_commision = array_sum($commision) + $this->qTransfer($type, $amount);
        }
        ...
        while ($uid != 0) {
            if ($this->isOwner($uid)) {
                ...
                $parent_type =  $this->uType($parent_id);
                if ($this->isPercent($type, $amount)) {
                    $main_commision = $this->qTransfer($type, $amount) * ($amount / 100);
                } else {
                    $main_commision = $this->qTransfer($type, $amount);
                }
                ...
                if ($this->isPercent($parent_type, $amount)) {
                    $parent_commision = ($this->qTransfer($parent_type, $amount) * ($amount / 100));
                } else {
                    $parent_commision = $this->qTransfer($parent_type, $amount);
                }
