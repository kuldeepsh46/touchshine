...
class dmt_model extends CI_Model
{
  ...
  {
    $ispercent = $this->db->get_where('dmtcharge', array('package' => $utype, 'froma<=' => $amount, 'toa>=' => $amount))->row()->percent;
    if ($ispercent == 0) {
      return false;
    } else {
      return true;
  ...
  public function dmtCharge($utype, $amount)
  {
    $dmtCharge = $this->db->get_where('dmtcharge', array('package' => $utype, 'froma<=' => $amount, 'toa>=' => $amount))->row()->amount;
    return $dmtCharge;
  ...
  public function charge($uid, $amount, $txnid, $site)
  {
      $amount= (float)$amount;
    $main_user=$uid;
    $da = $uid." ".$amount." ". $txnid." ". $site;
    //$this->db->insert('test',array('data' => $da));
    $commision = array();
    ...
    if ($this->isPercent($type, $amount)) {
      $main_commision = array_sum($commision) + ($this->dmtCharge($type, $amount) * ($amount / 100));
    } else {
      $main_commision = array_sum($commision) + $this->dmtCharge($type, $amount);
    }
    ...
    while ($uid != 0) {
      if ($this->isOwner($uid)) {
        ...
        $parent_type =  $this->uType($parent_id);
        if ($this->isPercent($type, $amount)) {
          $main_commision = $this->dmtCharge($type, $amount) * ($amount / 100);
        } else {
          $main_commision = $this->dmtCharge($type, $amount);
        }
        ...
        if ($this->isPercent($parent_type, $amount)) {
          $parent_commision = ($this->dmtCharge($parent_type, $amount) * ($amount / 100));
        } else {
          $parent_commision = $this->dmtCharge($parent_type, $amount);
        }
      ...
      foreach ($user_n_com as $key => $value) {
        ...
        if($key==$main_user){
          ...
          $data = ['uid' => $key, 'amount' => $value, 'txnid' => $txnid, 'opening' => $openingBal, 'closing' => $closingBal, 'site' => $site, 'type' => 'DMT-CHARGE', 'txntype' => $txntype];
          $checkTxn = $this->db->get_where('wallet', array('txnid' => $txnid, 'uid' => $key, 'txntype' => $txntype, 'type' => 'DMT-CHARGE'))->row()->txnid; //check if there is any entry available with same transaction id(txnid), so that we can save from doind clone entry
        if (!$checkTxn) {
